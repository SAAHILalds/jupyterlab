...
[packages/theme-light-extension/style/variables.css](https://github.com/jupyterlab/jupyterlab/blob/4.5.x/packages/theme-light-extension/style/variables.css).
...
- The top level `index.css` file is templated by `buildutils` as
  ...
  For imports that should not be added to `index.css`, update `SKIP_CSS` in `buildutils/src/ensure-repo.ts`.
